        ...
        for (WebSocketServer item : webSocketSet) {
            if(item.sid.equals(sid)){
                ...
                for (int i = 0; i < messageArray.size(); i++) {
                    JSONObject tempObject = messageArray.getJSONObject(i);
                    Message tempMessage = new Message();
                    if ("AI".equals(tempObject.getString("type"))) {
                        tempMessage = Message.ofSystem(tempObject.getString("message"));
                    } else if ("Human".equals(tempObject.getString("type"))) {
                        tempMessage = Message.of(tempObject.getString("message"));
                    }
                    messageList.add(tempMessage);
                }

                ConsoleStreamListener listener = new ConsoleStreamListener(item);
